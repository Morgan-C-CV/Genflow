...
def load_and_clean_data(file_path):
    ...
    # Use existing 'style' and 'lora' fields if they exist, otherwise fallback
    df['style'] = df['style'].fillna('')
    df['lora'] = df['lora'].fillna('none')
    
    df['prompt'] = df['prompt'].fillna('')
    # Enhanced prompt still uses extracted loras to maintain consistency
...
def build_features(df):
    ...
    print(f"Raw prompt feature dimensions: {text_features.shape}", flush=True)

    print("[Phase 1b] Extracting style features using CLIP model...", flush=True)
    style_features = model.encode(df['style'].tolist(), show_progress_bar=True)
    print(f"Raw style feature dimensions: {style_features.shape}", flush=True)
    ...
    return text_features, style_features, num_features, sampler_features, model_features, scaler, encoder_sampler

def perform_two_stage_pca(text_features, style_features, num_features, sampler_features, model_features, final_dim=8):
    print(f"\n[Phase 3] Performing two-stage PCA (Target dimensions: {final_dim})...", flush=True)
    
    # Process text/prompt features
    ...
    # Process style features (mimic prompt)
    n_style_components = min(20, style_features.shape[0], style_features.shape[1])
    pca_style = PCA(n_components=n_style_components)
    style_reduced = pca_style.fit_transform(style_features)
    scaler_style = StandardScaler()
    style_reduced_scaled = scaler_style.fit_transform(style_reduced)
    
    print(f"Phase 1: Prompt compressed to {n_text_components} dim, Style compressed to {n_style_components} dim", flush=True)
    ...
    combined_features = np.hstack([
        ...
        style_reduced_scaled * STYLE_WEIGHT,
...
def simulate_pbo_retrieval(pbo_space, df, target_index=0, top_k=5):
    ...
    for i, idx in enumerate(indices[0]):
        ...
        print(f"  - Model: {metadata_dict['model']} | Style: {metadata_dict['style']}", flush=True)
...
def run_pbo_loop(pbo_space, df, iterations=10, batch_size=4):
    ...
    while current_iter < iterations:
        ...
        for idx, c_idx in enumerate(candidate_indices):
            row = df.iloc[c_idx]
            print(f"  [{idx+1}] ID: {row.get('id', 'N/A')} | Style: {row.get('style', 'none')}", flush=True)
            print(f"      Prompt: {row['prompt'][:80]}...", flush=True)
    ...
    print(f"Model: {row.get('model', 'UNKNOWN')} | Style: {row.get('style', '')}", flush=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        text_features, style_features, num_features, sampler_features, model_features, scaler, encoder = build_features(df)
        pbo_space, pca_text, pca_final = perform_two_stage_pca(text_features, style_features, num_features, sampler_features, model_features, final_dim=8)
